...
- `headers-compose.js`: emits the layout. `emitReactFrameworkHeaders()` writes the
  React/react/bare-aliased headers into every slice's `React.framework/Headers`,
  and `buildReactNativeHeadersXcframework()` assembles the headers-only
  `ReactNativeHeaders.xcframework` carrying every non-React namespace plus the
  third-party dependency namespaces (`folly`, `glog`, `boost`, `fmt`,
  `double-conversion`, `fast_float`) and the Hermes public headers.
